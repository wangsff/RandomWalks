function [ DSIM_intra_full, param ] = load_dsim_intra_full(DB, param)
...
if ~exist(fullfile(param.dir.result_class, 'DSIM_intra_full.mat'), 'file')
    
    DSIM_intra_full = struct('indr', {}, ... row index
                        'indc', {}, ... col index
...
%                 edge_mat = edge_mat_ext;
                ...
                DSIM_intra_full(jIdx, iIdx).indr = indr;
                DSIM_intra_full(jIdx, iIdx).indc = indc;
                ...
                DSIM_intra_full(jIdx, iIdx).rgb = distances(ind_edge);
                ...
                DSIM_intra_full(jIdx, iIdx).lab = distances(ind_edge);
                ...
                DSIM_intra_full(jIdx, iIdx).boundary = distances(ind_edge);                
                ...
                DSIM_intra_full(jIdx, iIdx).BoW_rgb = distances(ind_edge);
                ...
                DSIM_intra_full(jIdx, iIdx).BoW_lab = distances(ind_edge);
    ...
    save(fullfile(param.dir.result_class, 'dsim_intra_full.mat'), 'DSIM_intra_full', 'eval');
    if param.disp.verbose
        fprintf('intra-image superpixel dissimilarities (full) are calculated and saved!\n');
    end
else
    load(fullfile(param.dir.result_class, 'dsim_intra_full.mat'));    
    if param.disp.verbose
        fprintf('intra-image superpixel dissimilarities (full) are loaded!\n');
    end
