function [ DSIM_intra, param ] = load_dsim_intra(DB, param)
...
if ~exist(fullfile(param.dir.result_class, 'dsim_intra.mat'), 'file')
    
    DSIM_intra = struct('indr', {}, ... row index
                        'indc', {}, ... col index
    ...
    for jIdx = 1:nImg
        ...
        for iIdx=1:nImg
            ...
            if jIdx==iIdx
                ...
                DSIM_intra(jIdx, iIdx).indr = indr;
                DSIM_intra(jIdx, iIdx).indc = indc;
                ...
                DSIM_intra(jIdx, iIdx).rgb = distances(ind_edge);
                ...
                DSIM_intra(jIdx, iIdx).lab = distances(ind_edge);
                ...
                DSIM_intra(jIdx, iIdx).boundary = distances(ind_edge);                
                ...
                DSIM_intra(jIdx, iIdx).BoW_rgb = distances(ind_edge);
                ...
                DSIM_intra(jIdx, iIdx).BoW_lab = distances(ind_edge);
    ...
    save(fullfile(param.dir.result_class, 'dsim_intra.mat'), 'DSIM_intra', 'eval');
    if param.disp.verbose
        fprintf('intra-image superpixel dissimilarities are calculated and saved!\n');
    end
else
    load(fullfile(param.dir.result_class, 'dsim_intra.mat'));    
    if param.disp.verbose
        fprintf('intra-image superpixel dissimilarities are loaded!\n');
    end
end
